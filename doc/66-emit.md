...
`emit` is the manual entry point into the [event system](66-event.html). The same *name* can also be raised from inside the system by another reactor's [`Emit`](66-event.html#emit) key; `66 emit` is the equivalent an operator or a script runs by hand. A *name* is an arbitrary label of at most 256 characters — it does not have to match a service name.
