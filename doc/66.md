...
```
66 [ -h ] [ -z ] [ -v verbosity ] [ -l live ] [ -T timeout ] [ -t tree ] start|stop|reload|restart|free|reconfigure|enable|disable|configure|status|log|resolve|state|runstate|remove|signal|emit|env|snapshot|wall|tree|parse|scandir|boot|poweroff|reboot|halt|suspend|hibernate|version [<command options> or subcommand <subcommand options>] service...|tree
```
...
- **-l, --live** *live*: changes the supervision directory of *service* to *live*. By default this will be `%%livedir%%`. The default can also be changed at compile time by passing the `-D livedir=` option to `meson setup`. An existing absolute path is expected and should be within a writable and executable filesystem - likely a RAM filesystem—see [scandir](66-scandir.html) command.
...
- [log](66-log.html): display service and system logs.
...
- [emit](66-emit.html): raise a user event to the event system.
- [env](66-env.html): publish per-session environment variables to services.
...
- [suspend](66-suspend.html): suspend the system to RAM.
- [hibernate](66-hibernate.html): hibernate the system to disk.
...
- [runstate](66-runstate.html): display the service's runtime status file contents.
...
An [instanced *service*](66-instantiated-service.html) name from a service template can be passed as service argument where the name of the *service* must end with a `@` (commercial at).—see [frontend service file](66-frontend.html).
