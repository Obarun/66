...
```
halt [ -h ] [ -a ] [ -c ] [ -f|F ] [ -m message ] [ -t time ] [ -W ] when
```
...
- **-c, --cancel**: cancel a planned halt. Any *when* argument is ignored.
...
- [the event system](66-event.html#lifecycle-events-raised-by-66): when the halt is scheduled — before any service is stopped — 66 raises the `shutdown.begin` event, which a service can react to in order to flush or quiesce cleanly during the grace period.
