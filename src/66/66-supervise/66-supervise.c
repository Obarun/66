 ...
 *   event broadcast      -> 66 event_fifo_* (lib66/event)
 ...
 * spaces disjoint); the event fifodir keeps the single transition bytes.
 *
...
#define USAGE "66-supervise /service/path"
 ...
 * The supervision scalars (notify/maxdeath/maxdeathtime) and the timeouts/downsignal
 * live in the autonomous execute addon, loaded into ex. */
...
static int fd_lock(int fd, int w, int nb)
{
    ...
    do {
        r = fcntl(fd, nb ? F_SETLK : F_SETLKW, &fl) ;
...
static void project_status(service_status_t *st)
{
    ...
    if (status.result == STATUS_RESULT_SIGNALED) {

        st->code = (uint32_t)WTERMSIG(status.wstat) ;

    } else if (status.result == STATUS_RESULT_EXITED) {

        st->code = (uint32_t)WEXITSTATUS(status.wstat) ;

    } else if (status.result == STATUS_RESULT_EXEC_FAILED) {

        st->code = (uint32_t)status.exec_errno ;
...
static inline void project_and_write(service_status_t *st)
{
    project_status(st) ;
...
/** write the status file only: a status change that is not a broadcastable edge
 * (pause/cont, or the intermediate flag flip of a composite start). */
static inline void announce_status(void)
{
    service_status_t st = STATUS_ZERO ;
    project_and_write(&st) ;
}

/** write the status file AND fan the projected transition out to the event
 * fifodir. @terminal sets EVENT_FLAG_TERMINAL on a down that will not restart. */
static inline void announce_event(uint8_t terminal)
{
    service_status_t st = STATUS_ZERO ;
    project_and_write(&st) ;
    if (!event_emit_transition(SS_EVENTDIR + 1, st.state, st.result, st.who, st.code, st.pid, &st.stamp, terminal))
      log_warnusys("emit event") ;
}
...
static void set_down_and_ready(uint8_t terminal)
{
    status.pid = 0 ;
    ...
    * would drop the pending newline and the service would never be seen ready. */
    for (;;) {
        ...
        if (r < 0) {
            notify_drop_req = 1 ;
            return ;
        } // EOF (EPIPE) or error: stop watching
...
static void enter_failed(void)
{
    ...
    announce_event(EVENT_FLAG_TERMINAL) ;
}
...
static void set_wantdown(void)
{
    if (status.flagwantup)
        latched_who = current_who ; // a real 1->0 owns the transition
...
static void downtimeout(void)
{
    ...
    if (ex.maxdeath && status.ndeaths >= ex.maxdeath) {
...
{
    set_wantdown() ; // flags only: trystart emits the single UP edge
    trystart() ;
}
...
static int uplastup_z(void)
{
    ...
    * Only an intrinsic death (still wanted up) is reported as SIGNALED/EXITED. */
    if (kill_timeout == 1) {
        ...
        status.result = STATUS_RESULT_TIMEOUT_START ;

    } else if (!status.flagwantup) {

        status.result = STATUS_RESULT_SUCCESS ;

    } else if (WIFSIGNALED(status.wstat)) {

        status.result = STATUS_RESULT_SIGNALED ;

    } else if (WEXITSTATUS(status.wstat)) {

        status.result = STATUS_RESULT_EXITED ;
...
{
    if (kill_timeout == 2)
        status.result = STATUS_RESULT_TIMEOUT_STOP ;
    ...
    if (WIFEXITED(finish_wstat) && WEXITSTATUS(finish_wstat) == 125) {
        ...
        status.flagwantup = 0 ;
        set_down_and_ready(EVENT_FLAG_TERMINAL) ; // ./finish asked not to be restarted

    } else set_down_and_ready(0) ;
}
...
/** the signal a control op routes to the service's process, or 0 for a non-signal
 * op. The twelve per-process ops come from the canonical map shared with `66 signal`
 * (ssexec_signal's cmdsig), so the two directions cannot drift; the group ops P/C/K
 * route the same signals to the process group. Reload ('l') is a Do=reload action,
 * not a signal reception, and is deliberately excluded. Only control-dispatched ops
 * reach here; the supervisor's own internal kills (up_d/uptimeout/finishtimeout)
 * never do, so they are never announced. */
static int op_signal(char op)
{
    switch (op) {
#define SVC_SIGNAL_CASE(o, sig) case o : return sig ;
        SVC_SIGNAL_MAP(SVC_SIGNAL_CASE)
#undef SVC_SIGNAL_CASE
        case 'P' : return SIGSTOP ; // --stop-group (killpg SIGSTOP)
        case 'C' : return SIGCONT ; // --cont-group (killpg SIGCONT)
        case 'K' : return SIGKILL ; // --kill-group (killpg SIGKILL)
        default  : return 0 ;
    }
}
...
static void dispatch_op(char op, uint8_t who)
{
    ...
    if (!p)
        return ;
    current_who = who ;
    state_t before = state ; // a signal op only fires (kills) while the service is up

    (*actions[state][V_a + (size_t)(p - control_alphabet)])() ;


    int signo = op_signal(op) ;

    if (signo && (before == UP || before == LASTUP)) {

        struct timespec now ;
        clock_now(&now) ;

          if (!event_emit_signal(SS_EVENTDIR + 1, (uint8_t)signo, who, &now))
            log_warnusys("emit signal") ;
    }
}

// control fifo: a fixed <op><who> pair per command
...
static void control_cb(sse_watcher_t *w, void *data, int revents)
{
    ...
    for (;;) {
        ...
        if (pending_op >= 0) {
            ...
            if (!memchr(control_alphabet, op, 25))
                continue ; // parasite or orphan who: ignore
        }
        ...
        if (!r) {
            pending_op = (unsigned char)op ;
            break ;
        } // who not here yet: buffer the op
...
static inline int control_init(void)
{
    ...
    if (!event_fifo_make(SS_EVENTDIR + 1, getegid()))
    ...
    if (fdlck < 0)
        log_dieusys(111, "open ", lock_file) ;

    r = fd_lock(fdlck, 1, 1) ;
    if (r < 0)
        log_dieusys(111, "lock ", lock_file) ;
    if (!r)
        log_die(100, "another instance of 66-supervise is already running") ;
...
int main(int argc, char const *const *argv)
{
    ...
    /** supervision scalars (notify/maxdeath/maxdeathtime) and the timeouts/downsignal
...

    {
        ...
        char progname[12 + namelen + 2] ;

        auto_strings(progname, PROG, " ", servicename) ;
        ...
        if (!event_fifo_clean(SS_EVENTDIR + 1))
            log_warnusys("clean ", SS_EVENTDIR + 1) ;

        if (access("down", F_OK) == 0){

            status.flagwantup = 0 ; // latched_who already defaults to SELF
        ...
        if (!event_emit_lifecycle(SS_EVENTDIR + 1, EVENT_LIFECYCLE_UP, &status.stamp))
            log_warnusys("emit supervise-up") ;
        ...
        while (gflags.cont) {
            ...
            do {
                sse_sanitize(&g_epoll) ;
            } while (g_epoll.rerun_file) ;

            if (!sse_prepare(&g_epoll))
            ...
            if (nfds < 0) {
                ...
                log_dieusys(111, "sse wait") ;

            } else if (!nfds) {

                (*actions[state][V_TIMEOUT])() ;
        ...
        struct timespec now ;
        clock_now(&now) ;
        event_emit_lifecycle(SS_EVENTDIR + 1, EVENT_LIFECYCLE_DOWN, &now) ;
    }
