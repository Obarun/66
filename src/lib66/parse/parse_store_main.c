...
static void io_compute_stdin(resolve_service_t *res, char const *line, uint32_t type)
{
    log_flow() ;
    ...
    switch(type) {
        ...
        default:
            res->io.fdin.type = E_PARSER_IO_TYPE_NOTSET ;
            break ;
    }
    free(wres) ;
}

static void io_compute_stdout(resolve_service_t *res, char const *line, uint32_t type)
{
    log_flow() ;
    ...
    switch(type) {
        ...
        case E_PARSER_IO_TYPE_INHERIT:
        default:
            res->io.fdout.type = E_PARSER_IO_TYPE_NOTSET ;
            break ;
    }
    free(wres) ;
}

static void io_compute_stderr(resolve_service_t *res, char const *line, uint32_t type)
{
    log_flow() ;
...
static int parse_io_type(resolve_service_t *res, char const *line, resolve_enum_table_t table)
{
    log_flow() ;
    ...
    switch(table.u.parser.id) {
        ...
        case E_PARSER_SECTION_MAIN_STDIN:
            io_compute_stdin(res, stk.s, (uint32_t)type) ;
            break ;

        case E_PARSER_SECTION_MAIN_STDOUT:
            io_compute_stdout(res, stk.s, (uint32_t)type) ;
            break ;

        case E_PARSER_SECTION_MAIN_STDERR:
            io_compute_stderr(res, stk.s, (uint32_t)type) ;
            break ;
...
int parse_store_main(resolve_service_t *res, stack *store, resolve_enum_table_t table)
{
    ...
    switch(kid) {
        ...
        case E_PARSER_SECTION_MAIN_STDERR:
            ...
            if (!parse_io_type(res, store->s, table))
                return 0 ;
            break ;
