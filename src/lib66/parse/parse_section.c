...
int parse_section_main(resolve_service_t *res, resolve_service_addon_execute_t *ex, resolve_service_addon_dependencies_t *dep, const char *str)
{
    resolve_enum_table_t table = E_TABLE_PARSER_SECTION_MAIN_ZERO ;
    return parse_section(res, ex, dep, str, table) ;
}

int parse_section_start(resolve_service_t *res, resolve_service_addon_execute_t *ex, resolve_service_addon_dependencies_t *dep, const char *str)
{
    resolve_enum_table_t table = E_TABLE_PARSER_SECTION_START_ZERO ;
    return parse_section(res, ex, dep, str, table) ;
}

int parse_section_stop(resolve_service_t *res, resolve_service_addon_execute_t *ex, resolve_service_addon_dependencies_t *dep, const char *str)
{
...
int parse_section(resolve_service_t *res, resolve_service_addon_execute_t *ex, resolve_service_addon_dependencies_t *dep, char const *str, resolve_enum_table_t table)
{
    log_flow() ;
    ...
    while (kcfg.pos < kcfg.slen) {
        ...
        if (kcfg.found) {
            ...
            if (!parse_store_g(res, ex, dep, &store, table))
