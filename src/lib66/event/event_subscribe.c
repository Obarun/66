...
int event_subscribe(event_fifo_t *f, sse_epoll_t *ep, char const *eventdir, event_handler_t *handler, void *data, int priority)
{
    log_flow() ;
    ...
    if (strlen(eventdir) + 2 + EVENT_FIFO_NAMELEN + 1 > sizeof(f->fifopath)) {
        errno = ENAMETOOLONG ;
        log_warnusys_return(LOG_EXIT_ZERO, "fit the event fifo path for: ", eventdir) ;
    }
     ...
     * which filters on the visible "evtsub:" prefix, never opens it before the
...
void event_unsubscribe(event_fifo_t *f)
{
    log_flow() ;
